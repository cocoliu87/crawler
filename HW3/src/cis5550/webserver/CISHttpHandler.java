    ...
    public synchronized void handle(HttpExchange exchange) throws IOException {
        ...
        for (Routing r: Server.rt) {
            if (exchange.getRequestMethod().equalsIgnoreCase(r.method.name())){
                if (exchange.getRequestURI().toString().toLowerCase().startsWith(r.pathPattern) ||
                        (r.pathPattern.contains(":") && exchange.getRequestURI().toString().toLowerCase().startsWith(r.pathPattern.split(":")[0]))) {
